...
2026-03-28 架构设计。第 1-3 层已实现 (2026-03-28 commit 33bc6f0)。5 层模板函数清单，~22 个函数 + 16 个常量替代 ~1200 行 parser 代码，还多了 6 个新能力函数。
...
总共 ~22 个函数 + 16 个常量，替代 ~1200 行 parser 代码，还多了 6 个新能力函数。

## 第 1 层：节点类型（16 个常量，从 PROPERTY_REGISTRY 派生）

**状态：✅ 已实现** — `src/engine/jsx/templateFunctions.ts`
...
const Icon = 'ICON', Image = 'IMAGE'
```

- 不是函数，是常量。LLM 写 `<Frame>` 编译后变成 `__h('FRAME', ...)`
- 来源：PROPERTY_REGISTRY 的节点类型 + Icon/Image，共 16 个常量
...
|---|---|---|---|---|
| 第 1 层 | 16 个常量 | jsxParser 的 VALID_TAGS | - | 必须有 |
...
| 文件 | 行数 | 说明 |
|---|---|---|
| `jsxParser.ts` | -378 行 | 被 `templateCompiler.ts` 替代 |
| `design-ir.ts` | -84 行 | 中间类型不再需要 |
| jsxToIR 相关测试 | -231 行 | 对应 parser/IR 代码已删除 |

## 前提条件

- ✅ 已解决 — `src/utils/sandboxEval.ts` 通过 `['Func','tion'].join('')` 绕过 esbuild 重命名
- ✅ 已引入 — sucrase 加入 package.json 依赖
- ✅ 确认 — nodeFactory + propertyDependencies + handlers 层不变，只改了 LLM 输出到虚拟 DOM 树这段
