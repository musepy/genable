...
import { validateNodeLayer, validateWithCoercion, coerceNodeLayer } from '../../../schema/layerSchema';
...
// ============================================================
// Part 4: Normalizer — structure healing + coercion + enum validation
// ============================================================

describe('Pipeline Stress: Normalizer', () => {
  it('heals array input by wrapping in FRAME container', () => {
    const input = [
  ...
  it('coerces string numbers to actual numbers', () => {
    ...
    const result = Normalizer.normalize(input);
    expect(result.props.width).toBe(360);
    expect(result.props.height).toBe(240);
    expect(result.props.gap).toBe(16);
    expect(result.props.cornerRadius).toBe(12);
    expect(result.props.fontSize).toBe(24);
    expect(result.props.paddingTop).toBe(10);
    expect(result.props.paddingRight).toBe(20);
    expect(result.props.paddingBottom).toBe(30);
    expect(result.props.paddingLeft).toBe(40);
  });

  it('normalizes type to uppercase', () => {
    const result = Normalizer.normalize({ type: 'frame', props: {} });
    expect(result.type).toBe('FRAME');
  });

  it('defaults missing type to FRAME', () => {
    const result = Normalizer.normalize({ props: {} });
    expect(result.type).toBe('FRAME');
  });
  ...
  it('validates and normalizes enum values', () => {
    const input = {
      type: 'FRAME',
      props: {
        layoutMode: 'vertical',       // lowercase → VERTICAL
        primaryAxisAlignItems: 'center', // lowercase → CENTER
        strokeAlign: 'outside',        // lowercase → OUTSIDE
      },
    };
    const result = Normalizer.normalize(input);
    expect(result.props.layoutMode).toBe('VERTICAL');
    expect(result.props.primaryAxisAlignItems).toBe('CENTER');
    expect(result.props.strokeAlign).toBe('OUTSIDE');
  });
  ...
  it('deletes invalid enum values', () => {
    const input = {
      type: 'FRAME',
      props: {
        layoutMode: 'INVALID_VALUE', // not in enumMap → deleted
      },
    };
    const result = Normalizer.normalize(input);
  ...
  it('normalizes constraint aliases (LEFT→MIN, BOTTOM→MAX)', () => {
    const input = {
      type: 'FRAME',
      props: {
    ...
    const result = Normalizer.normalize(input);
    expect(result.props.constraints.horizontal).toBe('MIN');
    expect(result.props.constraints.vertical).toBe('MAX');
  });

  it('normalizes constraint alias LEFT_RIGHT→STRETCH', () => {
    const input = {
      type: 'FRAME',
      props: {
    ...
    const result = Normalizer.normalize(input);
    expect(result.props.constraints.horizontal).toBe('STRETCH');
    expect(result.props.constraints.vertical).toBe('STRETCH');
  });

  it('uppercases semantic field with underscore replacement', () => {
    const input = {
      type: 'FRAME',
      props: { semantic: 'primary button' },
    };
    const result = Normalizer.normalize(input);
    expect(result.props.semantic).toBe('PRIMARY_BUTTON');
  });

  it('normalizes fills array with mixed formats', () => {
    const input = {
      type: 'FRAME',
      props: {
        fills: [
          '#FF0000',
          { color: '#00FF00' }, // legacy wrapper → extract
          { type: 'GRADIENT_LINEAR', stops: [{ position: 0, color: '#000' }, { position: 1, color: '#FFF' }] },
        ],
      },
    };
    const result = Normalizer.normalize(input);
    expect(result.props.fills).toHaveLength(3);
    expect(result.props.fills[0]).toBe('#FF0000');
    expect(result.props.fills[1]).toBe('#00FF00'); // extracted from wrapper
    expect(result.props.fills[2].type).toBe('GRADIENT_LINEAR'); // gradient preserved
  });

  it('handles position object fallback to x/y', () => {
    const input = {
      type: 'FRAME',
    ...
    expect(result.props.y).toBe(200);
    expect(result.props.position).toBeUndefined();
  });
  ...
  it('recursively normalizes children', () => {
    const input = {
      ...
      children: [
        {
          type: 'text',
          props: { fontSize: '16', textAlignHorizontal: 'center' },
        },
      ],
    };
    const result = Normalizer.normalize(input);
    ...
    expect(result.children[0].props.fontSize).toBe(16);
    expect(result.children[0].props.textAlignHorizontal).toBe('CENTER');
  });

  it('removes empty children array', () => {
    const input = { type: 'FRAME', props: {}, children: [] };
    const result = Normalizer.normalize(input);
  ...
  it('handles null/undefined input gracefully', () => {
    ...
    expect(result.type).toBe('FRAME');
    expect(result.props.name).toBe('Node');
  });
});
...
describe('Pipeline Stress: LayerSchema validation', () => {
  ...
  // ── validateWithCoercion: Normalizer + Schema ──

  it('validateWithCoercion normalizes + validates a messy LLM input', () => {
    const messyInput = {
      type: 'frame',       // lowercase → FRAME
      props: {
        name: 'Dashboard',
        width: '400',       // string → 360
        layoutMode: 'vertical', // lowercase → VERTICAL
        primaryAxisAlignItems: 'center', // lowercase → CENTER
        cornerRadius: '16', // string → 16
        fontSize: '24',     // string → 24
        fills: ['#FFFFFF'],
        semantic: 'main card', // → MAIN_CARD
        constraints: { horizontal: 'LEFT', vertical: 'TOP' }, // → MIN, MIN
      },
      children: [
        {
          type: 'text',
    ...
    const result = validateWithCoercion(messyInput);
    expect(result.success).toBe(true);
    expect(result.data!.type).toBe('FRAME');
    expect(result.data!.props.width).toBe(400);
    expect(result.data!.props.layoutMode).toBe('VERTICAL');
    expect(result.data!.props.primaryAxisAlignItems).toBe('CENTER');
    expect(result.data!.props.cornerRadius).toBe(16);
    expect(result.data!.props.semantic).toBe('MAIN_CARD');
    expect(result.data!.props.constraints.horizontal).toBe('MIN');
    expect(result.data!.children[0].type).toBe('TEXT');
    expect(result.data!.children[0].props.fontSize).toBe(16);
    expect(result.data!.children[0].props.textAlignHorizontal).toBe('CENTER');
  });
});
