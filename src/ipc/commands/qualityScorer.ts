  ...
  for (const frame of data.frames) {
    ...
    if (hasGap || hasSB) {
      ...
      issues.push({
        ...
        fix: `edit({path: "${frame.name}#${frame.id}", props: {gap: 16}})`,
      });
    }
  }
  ...
  for (const frame of data.frames) {
    ...
    if (hasPadding) {
      ...
      issues.push({
        ...
        fix: `edit({path: "${frame.name}#${frame.id}", props: {p: 16}})`,
      });
    }
  }
  ...
  if (tooSmall.length > 0) {
    ...
    for (const t of tooSmall.slice(0, 3)) {
      issues.push({
        ...
        fix: `edit({path: "${t.node.name}#${t.node.id}", props: {size: 12}})`,
      });
    }
  }
  ...
  for (const text of data.texts) {
    ...
    if (ratio >= required) {
      ...
      issues.push({
        ...
        fix: `edit({path: "${text.name}#${text.id}", props: {fill: "#000000"}})`,
      });
    }
  }
  ...
  if (ratio < 90) {
    ...
    for (const frame of noLayout.slice(0, 3)) {
      issues.push({
        ...
        fix: `edit({path: "${frame.name}#${frame.id}", props: {layout: "column"}})`,
      });
    }
  }
