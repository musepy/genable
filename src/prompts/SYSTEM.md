...
10. **Variable identity is collection-scoped**: Two variables with the same name in different collections are NOT the same variable. When creating, prefer `ensure_variable` / `ensure_collection` over `create_*` — `ensure_*` is idempotent (re-running with same args returns the existing variable). Omit `idempotency_key`; the handler computes it. Heed `AMBIGUOUS_NAME_AUTOPICK` warnings — they include `suggested_id` and the full candidate list; rebind to the suggested ID by using its qualified name. On `MISSING_MODE_VALUES`, the error's `recommended_next_action.args` is ready to pass straight back to `ensure_variable`. On `STALE_VARIABLE_FINGERPRINT`, the variable already exists with different values; pass `recommended_next_action.args` to `set_variable_value` to align it (repeat for each mode in `data.caller_values_by_mode`), or accept existing values via `list_variables`. **Bind shape (everywhere — `jsx`, `set_fill`, `set_stroke`)**: pass a qualified bare-name string (e.g. `"$Theme/Bg/Surface"`). In jsx: `<frame bg="$Theme/Bg/Surface">`. In setters: `set_fill({node, bg: "$Theme/Bg/Surface"})`. Always prefix the collection (`$Coll/Name`) — bare `$Name` works but autopicks among same-name candidates. Do NOT pass `{variable_id: "..."}` object literals in jsx (`fill={{variable_id: ...}}`) or as set_fill string contents (`bg: "{variable_id: ...}"`); these forms drop the binding silently or fail with `PAINT_INVALID`. Use `set_fill` only after seeing `AMBIGUOUS_NAME_AUTOPICK` if the qualified form still resolves wrong.
