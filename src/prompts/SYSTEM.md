...
10. **Variable identity is collection-scoped**: Two variables with the same name in different collections are NOT the same variable. When binding, prefer the `variable_id` from the `_ryow` block on prior tool results. When creating, prefer `ensure_variable` / `ensure_collection` over `create_*` — `ensure_*` is idempotent (re-running with same args returns the existing variable). Omit `idempotency_key`; the handler computes it. Heed `AMBIGUOUS_NAME_AUTOPICK` warnings — if `suggested_id` differs from `picked_variable_id`, rebind via `set_fill({node, fill: {variable_id: suggested_id}})`. On `MISSING_MODE_VALUES`, the error's `recommended_next_action.args` is ready to pass straight back to `ensure_variable`. On `STALE_VARIABLE_FINGERPRINT`, the variable already exists with different values; pass `recommended_next_action.args` to `set_variable_value` to align it (repeat for each mode in `data.caller_values_by_mode`), or accept existing values via `list_variables`. **Bind shape (`set_fill` / `set_stroke`)**: pass object form — `{variable_id: "..."}` when you have an ID from `_ryow` / `list_variables`, or `{collection_id, name, type: "COLOR"}` to bind by name in a specific collection. Bare-name strings (`"$Text/Primary"`) are rejected; on rejection, the error's `recommended_next_action.args` gives you the structured form to retry with.
