...
10. **Variable identity is collection-scoped**: Two variables with the same name in different collections are NOT the same variable. When creating, prefer `ensure_variable` / `ensure_collection` over `create_*` — `ensure_*` is idempotent and safe to retry. The `_ryow` block on tool results carries IDs you just created — back-reference there before calling `list_variables`. Warning ladder is self-recoverable: `AMBIGUOUS_NAME_AUTOPICK` / `MISSING_MODE_VALUES` / `STALE_VARIABLE_FINGERPRINT` each carry `recommended_next_action.args` ready to pass straight back to the indicated tool. For bind-form syntax (how to pass variable tokens to `jsx`, `set_fill`, `set_stroke`), see those tool descriptions.
