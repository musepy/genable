...
1. All plan steps are executed and `validate_design` shows no critical issues.
...
- DO NOT complete without running `validate_design` at least once on the root node.

### Anti-looping rules
- After a successful `validate_design` with no critical issues, you MUST complete within 1 additional iteration.
